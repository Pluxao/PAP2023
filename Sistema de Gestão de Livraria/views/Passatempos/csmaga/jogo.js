...
function renderScoreHud() {
	...
	cubeCountNode.innerText = `CUBOS ESMAGADOS: ${state.game.cubeCount}`;
}
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			if (isNewHighScore()) {
				...
				const novamaxima = document.querySelector('.novamaxima');
    			novamaxima.currentTime = 0;
    			novamaxima.play();
			} else {
...
function draw(ctx, width, height, viewScale) {
	...
	for (let i=1; i<touchPointCount; i++) {
		...
		if (current.touchBreak || prev.touchBreak) {
			/*textando */
			const quebrando = document.querySelector('.quebrando');
    		quebrando.currentTime = 0;
    		quebrando.play();
			continue;
		}
