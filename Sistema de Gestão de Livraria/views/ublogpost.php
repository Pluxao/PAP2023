    ...
    require 'user.php';
        ...
        if($id_post){
         ...
         $sql->execute();
             ...
             if($post->rowCount() > 0){
                 ...
                 echo "<script>window.location = 'ublog.php'</script>";
             }
         }else{
                 ...
                 if($sql->rowCount() > 0){
                     if (!empty($_GET['search'])){
                         ...
                         if($sql->rowCount() > 0){
                             ...
                             echo "<script>window.location = 'ublogpost.php?id_post=$id_post'</script>";
                         }
                     }else{
                         $sql = $pdo->prepare("SELECT *FROM comentarios, posts, usuarios WHERE comentarios.cod_post = posts.id_post AND comentarios.comentador = usuarios.id AND id_post = :id_post");
                         $sql->bindValue(':id_post', $id_post);
        ...
        $listapost = [];
        $posts = $pdo->query("SELECT *FROM posts order by id_post DESC limit 6");
        if($posts->rowCount() > 0){
        ...
        <div class="humberger__menu__wrapper">
            ...
            <nav class="humberger__menu__nav mobile-menu">
                <ul>
                    ...
                    <li class="active"><a href="./ublog.php">BLOG</a></li>
                    ...
                    <div class="col-lg-9">
                        <div class="hero__search">
                            <div class="hero__search__form">
                                <div>
                                    <div style="width: 30%; float: left;ont-size: 16px;color: #1c1c1c;ont-weight: 700;padding-left: 18px;padding-top: 11px; position: relative;" class="hero__search__categories">
                            ...
                            <div class="row">
                                <div class="col-lg-5">
                                    <div class="blog__details__author">
                                        ...
                                        <img src="../views/Painel/views/img_upload/<?php echo $consulta['imagem_perfil']?>"alt="..." class="avatar-img rounded-circle">
                                        </div>
                                    ...
                                    <?php
                                        ...
                                        if (isset($_POST['postar'])) {
                                            if ($cod_post && $comentador && $comentario) {
                                                ...
                                                echo "<script>window.open('ublogpost.php?id_post=$postagem[id_post]', '_self')</script>";
                                                exit();

                                            }else{
                                                echo "<script>alert('O comentário está vazio, escreva alguma coisa!')</script>";
                                                echo "<script>window.open('ublog.php', '_self')</script>";
                                                exit();
                                            }
                                        }
                                    ?>
        ...
        <section class="d-flex;" style="align-items: left;">
            ...
            <div class="d-flex container" style="align-items: center; margin: 2%;">
                ...
                <?=$comit['pnome'];?>&nbsp;<?=$comit['unome'];?>&nbsp;&nbsp;<?php 
                        if($comit['comentador'] == $consulta['id']){echo '<a style="color: #9b111e;" href="uapagarcomentario.php?id_comentario='.$comit['id_comentario'].'"><i style="font-size: 20px;" class="fa fa-trash-o" aria-hidden="true"></i></a>';}?></p>
                    <p style="color: black;">&nbsp;<?=$comit['comentario'];?>
                </div>
            </div>
            <?php endforeach;?>
        </section>
...
function searchLivros() {
    window.location = "ublogpost.php?id_post=<?=$id_post;?>&search="+search.value; 
}
    </script>
