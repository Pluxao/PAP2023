...
if (!empty($_GET['search'])) {
	$lista = [];
	$data = $_GET['search'];
	$sql = $pdo->query("SELECT *FROM baixados, usuarios WHERE baixados.iduser = usuarios.id AND (identificador LIKE '%$data%' or iduser LIKE '%$data%' or nome_baixado LIKE '%$data%' or hora_baixado LIKE '%$data%' or pnome LIKE '%$data%' or unome LIKE '%$data%') ORDER BY identificador");
	if ($sql->rowCount() > 0) {
		$lista = $sql->fetchALL(PDO::FETCH_ASSOC);
	} else {
		echo "<script>alert('Resultado não encontrado')</script>";
		echo "<script>window.location = 'baixados.php'</script>";
	}
} else {
	$lista = [];
	$sql = $pdo->query("SELECT *FROM baixados, usuarios WHERE baixados.iduser = usuarios.id");
	if ($sql->rowCount() > 0) {
		$lista = $sql->fetchALL(PDO::FETCH_ASSOC);
	}
}
...
<body>
				...
				<div class="page-inner">
					<div class="page-header">
						...
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="baixados.php">Baixados</a>
							</li>
						</ul>
					</div>
					<div class="col-md-12">
						<div class="card">
							<div class="card-header">
								<div class="d-flex align-items-center">
									...
									<h4 class="btn-round ml-auto">Livros Baixados</h4>
								</div>
							</div>
							<div class="card-body">
								<div class="row">
									<div class="table-responsive">
										...
										<table id="add-row" class="display table table-striped table-hover">
											<tr class="text-center">
												<th>ID do Baixador</th>
												<th>Nome do Baixador</th>
											...
											<?php foreach ($lista as $usuario): ?>
												<tr class="text-center">
													...
													<td>
														<?= $usuario['titulo_baixado']; ?>
													</td>
													<td>
														<?= $usuario['cod_baixado']; ?>
													</td>
													<td>
														<?= $usuario['disponibilidade_baixado']; ?>
													</td>
													<td>
														<?= number_format($usuario['preco_baixado'], 2, ",", ".");
														echo "&nbsp;AOA" ?>
													</td>
													<td>
														<?= $usuario['hora_baixado']; ?>
													</td>
												</tr>
											<?php endforeach; ?>
											</tbody>
	...
	<script>
		...
		function searchData() {
			window.location = "baixados.php?search=" + search.value;
		}
