<?php
	 ...
	 if (!empty($_GET['search'])){
		$lista = [];
		$data = $_GET['search'];
		$sql = $pdo->query("SELECT *FROM usuarios WHERE estado like 'Admin' AND (pnome LIKE '%$data%' or unome LIKE '%$data%') ORDER BY id");
		if($sql->rowCount() > 0){
        	$lista = $sql->fetchALL(PDO::FETCH_ASSOC);
    	}else{
			echo "<script>alert('Resultado não encontrado')</script>";
			echo "<script>window.location = 'despromover.php'</script>";
		}
		
	}
	 else{
		$lista = [];
    	$sql = $pdo->query("SELECT *FROM usuarios WHERE estado like 'Admin'");
    	if($sql->rowCount() > 0){
        	$lista = $sql->fetchALL(PDO::FETCH_ASSOC);
    	}
	 }
...
<head>
	...
	<title>Despromover usuários</title>
...
<div class="wrapper">
						...
						<li class="nav-item">
							...
							<div class="collapse" id="tables">
								<ul class="nav nav-collapse">
									<li>
										<a href="promover.php">
											<span class="sub-item">Promover Utilizador</span>
										</a>
									</li>
									<li>
										<a href="despromover.php">
											<span class="sub-item">Despromover Utilizador</span>
										</a>
									</li>
									<li>
										<a href="../../logout.php">
								...
								<div class="card-body">
									...
									<div class="table-responsive">
										<div class="d-flex box-search">
											<input type="search" id="pesquisar" placeholder="Pesquisar Usuários" style="padding: 1%; border-radius: 4px;">
											<button onclick="searchData()" style="background-color:rgba(45,128,234); border-radius: 4px; border-color:rgba(45,128,234); padding: 1%;"><i class="fas fa-search" style="width:30px; color: white;"></i></button>
										</div>
										<table id="add-row" class="display table table-striped table-hover">
											...
											<?php foreach($lista as $usuario): ?>
												<tr>
													<td><?=$usuario['pnome'];?>&nbsp;&nbsp;<?=$usuario['unome'];?></td>
													<td><?=$usuario['email']; ?></td>
													<td><?=$usuario['estado']; ?></td>
													<td>
														<div class="form-button-action" style="background: #1465c7; border-radius: 4px;">
																<form action="despromover_action.php" method="post">
																	<input type="text" name="id" value="<?=$usuario['id'];?>" hidden>
																	<input type="text" name="estado" value="Utilizador" hidden>
																	<button type="submit" data-toggle="tooltip" title="" class="btn btn-link btn-primary btn-lg" data-original-title="Tornar utilizador normal"><i style="color: white;" class="fa fa-arrow-down" aria-hidden="true"></i></button>
																</form>
														</div>
													</td>
												</tr>
	...
	<script >
		...
		function searchData() {
    		window.location = "despromover.php?search="+search.value; 
		}
