<?php
    ...
    if(isset($_POST["submit"])) {
        ...
        if($titulo_post && $post && $id_post){
            $sql = $pdo->prepare("UPDATE posts SET titulo_post = :titulo_post, post = :post WHERE id_post = :id_post");
            $sql->bindValue(':titulo_post', $titulo_post);
            $sql->bindValue(':post', $post);
            $sql->bindValue(':id_post', $id_post);
            $sql->execute();

            echo "<script>alert('A postagem foi Editada!')</script>";
								...
								<div class="row py-3">
									<section class="cadastrar">
        							    <div class="cadastrar_texto">
											...
											<h2>Ao editar a postagem, queira por favor verificar os <a href="../../atermos.php" style="text-decoration:none;">Termos de uso</a> para não lhe ser aplicado uma sansão</h2> 
                                        </div>
		                                    ...
		                                    <form method="post">
												...
												<div class="col-md-12">
				                                        <div class="form-group form-group-default">
					                                        <label>Título</label>
					                                        <input id="addOffice" type="text" value="<?php echo$post['titulo_post'];?> "class="form-control" name="titulo_post">
				                                        </div>
			                                        </div>
			                                        <div class="col-md-12">
				                                        <div class="form-group form-group-default">
					                                        <label>Corpo da publicação</label>
					                                        <textarea class="editar" style="padding: 2%;" name="post" id="addOffice" value="<?php echo$post['post'];?>" cols="30" rows="10"><?php echo$post['post'];?></textarea>
				                                        </div>
			                                        </div>
			                                        <input type="text" name="id_post" hidden value="<?php echo$post['id_post'];?>">
		                                        </div>
												<div class="col-md-12">
			                                    	<button type="submit" name="submit" class="primary-btn">Publicar</button> 
			                                    </div>
                                            </form>
                                        </div>
                                    </section>
