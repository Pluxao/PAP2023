<?php
	...
	if (!empty($_GET['search'])){
		$lista = [];
		$data = $_GET['search'];
		$sql = $pdo->query("SELECT *FROM usuarios WHERE estado like 'Utilizador' AND (pnome LIKE '%$data%' or unome LIKE '%$data%') ORDER BY id");
		if($sql->rowCount() > 0){
        	$lista = $sql->fetchALL(PDO::FETCH_ASSOC);
    	}else{
			echo "<script>alert('Resultado não encontrado')</script>";
			echo "<script>window.location = 'promover.php'</script>";
		}
		
	}
	 else{
		$lista = [];
    	$sql = $pdo->query("SELECT *FROM usuarios WHERE estado like 'Utilizador'");
    	if($sql->rowCount() > 0){
        	$lista = $sql->fetchALL(PDO::FETCH_ASSOC);
    	}
	 }
...
<div class="wrapper">
												...
												<tr>
													...
													<td>
														<div class="form-button-action" style="background: #1465c7; border-radius: 4px;">
																<form action="promover_action.php" method="post">
																	<input type="text" name="id" value="<?=$usuario['id'];?>" hidden>
																	<input type="text" name="estado" value="Admin" hidden>
																	<button type="submit" data-toggle="tooltip" class="btn btn-link btn-primary btn-lg" data-original-title="Tornar Admin"><i style="color: white;" class="fa fa-arrow-up" aria-hidden="true"></i></button>
																</form>
														</div>
													</td>
												</tr>
	...
	<script >
		...
		function searchData() {
    		window.location = "promover.php?search="+search.value; 
		}
