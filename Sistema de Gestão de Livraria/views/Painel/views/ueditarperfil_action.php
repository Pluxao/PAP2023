<?php
   ...
   require '../../user1.php';
    ...
    if($user && $pnome && $unome && $email && $telefone && $nasc && $genero){
        ...
        if($sql->rowCount() === 1){
        $sql = $pdo->prepare("UPDATE usuarios SET user = :user, pnome = :pnome, unome = :unome, email = :email, telefone = :telefone, nasc = :nasc, genero = :genero WHERE id = :id");
        $sql->bindValue(':user', $user);
        ...
        echo "<script>window.open('./ueditarperfil.php', '_self')</script>";
        exit();
    }
}
else{
