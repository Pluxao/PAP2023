<?php
    ...
    if($user && $pnome && $unome && $estado && $email && $telefone && $nasc && $genero && $senha && $confsenha){
        ...
        if($sql->rowCount() === 0){
            if($senha == $confsenha){
                ...
                $sql = $pdo->prepare("INSERT INTO usuarios (user, pnome, unome, estado, email, telefone, nasc, genero, senha, confsenha) VALUES(:user, :pnome, :unome, :estado, :email, :telefone, :nasc, :genero, :senha, :confsenha)");
                $sql->bindValue(':user', $user);
                ...
                echo "<script>window.open('painel/views/usuarios.php', '_self')</script>";
                exit();
            }
            else{
